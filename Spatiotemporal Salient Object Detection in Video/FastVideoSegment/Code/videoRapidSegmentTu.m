function SegmentationCom = videoRapidSegmentTu(options, params, data)
...
%     superpixels = data.superpixels;
    ...
    data.inMapsCom = TgetInOutMaps(flow, magflow, Images, Orgflow);    % Orgflow; flow
    data.NoOrgLab = false;
    
    for iS = 1:size(data.inMapsCom,3)
        superpixels = data.superpixels;
        data.inMaps = data.inMapsCom(:,:,iS);
        ...
        for frame = 1: frames        
            ...
            if( size( uniqueFgColours, 1 ) < fgMix || size( uniqueBgColours, 1 ) < bgMix )
                ...
                [fgModel, MaxSglarLab] = fitGMM( fgMix, uniqueFgColours, fgNodeWeights );
                MaxSglarNum = MaxSglarNum + MaxSglarLab;
        ...
        for i = 2: params.maxIterations 
            ...
            if( options.vocal ), tic; fprintf( '\n videoRapidSegment: Iteration: %i...\t\t\t', i ); end
            ...
            if( all( labels ) || all( ~labels ) )
                if options.vocal
                    fprintf( '\nvideoRapidSegment: Trivial segmentation detected, exiting...\n' ); 
                end
                return;
            end        
        end
        ...
        if iS == 1
            SegmentationCom = segmentation;
        else
            SegmentationCom = cat(3, SegmentationCom, segmentation);
        end
