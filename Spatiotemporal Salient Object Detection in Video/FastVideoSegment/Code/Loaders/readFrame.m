...
%     if( options.stored )
%         filename = fullfile( options.infolder, sprintf('%08d.jpg', index ) ); %%08d
...
%     end
    ...
    if exist(fullfile(folderName, [frameName '.png']),'file')
        frame = imread(fullfile(folderName, [frameName '.png']));
    elseif exist(fullfile(folderName, [frameName '.jpg']),'file')
        frame = imread(fullfile(folderName, [frameName '.jpg']));
    elseif exist(fullfile(folderName, [frameName '.bmp']),'file')
        frame = imread(fullfile(folderName, [frameName '.bmp']));
    end
