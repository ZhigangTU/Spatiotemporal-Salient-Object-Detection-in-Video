...
function segmentation = videoRapidSegment_M( options, params, data )
    ...
    SaliencyUnary = [cell2mat(data.saliency),cell2mat(data.saliency)];
    %SaliencyUnary = 255*SaliencyUnary
    SaliencyUnary(foregroundMasks,1) = -log(SaliencyUnary(foregroundMasks));
    SaliencyUnary(backgroundMasks,1) = 0;
    SaliencyUnary(backgroundMasks,2) = -log(1 - SaliencyUnary(backgroundMasks));
    SaliencyUnary(foregroundMasks,2) = 0;
        ...
        if( size( uniqueFgColours, 1 ) < fgMix || ...
           ...
           %% CHANGE UNARY PROTENTIAL FUNCTION 10-12-ME
            ...
            unaryPotentials( startIndex: stopIndex, : ) = ...
                SaliencyUnary( startIndex: stopIndex,:) + ...
