...
function segmentation = videoRapidSegment_Y( options, params, data )
...
%     temp = cellfun(@graythresh,data.saliency,'UniformOutput',false);
%     for i = 1 : length(temp)
%         temp{i} = data.saliency{i} > 0.5*temp{i};
%     end
%     
%     
%     data.inMaps = temp;
...
%     inRatios = getSuperpixelInRatio( superpixels, data.inMaps );
    ...
    %% UNARY TERM FOR SALIENCY MAP 10-12-ME
    SaliencyUnary = cell2mat(data.saliency);
    %SaliencyUnary = 255*SaliencyUnary
    SaliencyUnary(foregroundMasks) = -log(SaliencyUnary(foregroundMasks));
    SaliencyUnary(backgroundMasks) = -log(1 - SaliencyUnary(backgroundMasks));
    SaliencyUnary = [SaliencyUnary, (1 - SaliencyUnary)];
    %SaliencyUnary = zeros(size(SaliencyUnary));
