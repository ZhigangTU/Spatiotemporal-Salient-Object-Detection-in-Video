/*******************************************************************************
% Code originally written by Piotr Dollar, 2014, 
% for Structured Edge Detection Toolbox Version 3.01
% Modified by Philippe Weinzaepfel, 2015, for motion boundaries detection
...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  const int nChnsColor = (int) mxGetScalar(mxGetField(opts,0,"nChnsColor")); 
  ...
  if( sharpen && Z>0 ) {
    // erase everything in the image which is not a color
    memset(I + nChnsColor*w*h, 0, (Z-nChnsColor)*w*h*sizeof(I[0]));
