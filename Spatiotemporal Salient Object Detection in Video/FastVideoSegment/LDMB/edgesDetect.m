...
%
% 
% Code written by Piotr Dollar, 2014, for Structured Edge Detection Toolbox Version 3.01
% Licensed under the MSR-LA Full Rights License [see license.txt]

% get parameters
opts=model.opts; opts.nTreesEval=min(opts.nTreesEval,opts.nTrees);
if(~isfield(opts,'sharpen'))
    opts.sharpen=0; 
end
if(~isfield(model,'segs'))
    model.segs=[]; 
    model.nSegs=[]; 
end
