...
#define USAGE_NOTE "TgetSuperpixelsStats USAGE:\n" \
"\tExpected inputs:\n" \
"\t\t1 - Frames: a cell array of length frames. Each cell i contains a HxWx3 " \
"double matrix containing the color values of frame i.\n" \
"\t\t2 - Superpixels: a cell array of length frames. Each cell i contains a " \
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
	...
	#ifdef DEBUG_MODE
	mexPrintf( "TgetSuperpixelsStats: Number of frames: %i\n", frames );
	mexPrintf( "TgetSuperpixelsStats: Frame size: %ix%i\n", height, width );
	mexPrintf( "TgetSuperpixelsStats: Total number of superpixels: %i\n", superpixels );
	mexEvalString( "pause(0.001)" );
	#endif
	
	#ifdef DEBUG_MODE
	mexPrintf( "TgetSuperpixelsStats: Allocating memory for outputs...\n" );
	mexEvalString( "pause(0.001)" );
	#endif
	...
	#ifdef DEBUG_MODE
	mexPrintf( "TgetSuperpixelsStats: Computing superpixel stats...\n" );
	mexEvalString( "pause(0.001)" );
	#endif
		...
		for( unsigned int i = 0; i < height; i++ )
		{
			for( unsigned int j = 0; j < width; j++ )
			{
				...
				if( superpixel < 0 && superpixel >= superpixels )
					mexErrMsgTxt( "TgetSuperpixelsStats: Superpixel labels outside given range" );
