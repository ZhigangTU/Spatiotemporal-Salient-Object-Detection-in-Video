function  [ ConPix0, ConPix1, ConPixSecond ] = find_connect_superpixel_DoubleIn_Opposite(labels, K, height ,width )
%%
...
for i=1:height-1
    for j=1:width-1
        if labels(i,j)~=labels(i,j+1)
            ConPix(labels(i,j) ,labels(i,j+1) )=1;
        end
        if labels(i,j)~=labels(i+1,j)
            ConPix(labels(i,j) ,labels(i+1,j) )=1;
        end
    end
...
for j=1:width-1
    if labels(height,j)~=labels(height,j+1)
        ConPix( labels(height,j),labels(height,j+1) )=1;
    end
end
for i=1:height-1
    for j=1:width-1
        if labels(i,j)~=labels(i+1,j+1)
            ConPix( labels(i,j),labels(i+1,j+1) )=1;
        end
    end
end
for i=1:height-1
...
for j=1:width 
    ConPix( labels(1,j) , labels(height,j) ) = 1;
end
for j=1:height
    ConPix( labels(j,1) , labels(j,width) ) = 1 ;
end
ConPix=ConPix+ConPix';

ConPix1=ConPix+ConPix';
