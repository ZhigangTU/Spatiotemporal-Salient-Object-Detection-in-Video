...
userName = 'treeman120leftdown' ;


% output path
if methodchoose==8
    destRoot = fullfile(currentPath,'gzfixblocktest') ;
end   
...
% 2nd ronund decomposition
%data=A_hat;
if methodchoose==8
    [A_hat E_hat iter] = gzfixagainblock2and1norm_inexact_alm_rpca(data);
end 

AANames = fullfile(destDir, 'Amatrix2.mat'); 
save(AANames,'A_hat') ;
EENames = fullfile(destDir, 'Ematrix2.mat'); 
save(EENames,'E_hat') ;
...
for fileIndex = 1:numImages
    ...
    outputFileNameLowrank  = sprintf('Lowrankagain%05d.jpg',fileIndex);
    outputFileNamesLowrank = fullfile(destDir, outputFileNameLowrank); 
    imwrite(im1, outputFileNamesLowrank);
    ...
    outputFileNameSparse  = sprintf('Sparseagain%05d.jpg',fileIndex);
    outputFileNamesSparse= fullfile(destDir, outputFileNameSparse); 
    %imwrite(im2, outputFileNamesSparse);
    imwrite(uint8(im2), outputFileNamesSparse);
end
