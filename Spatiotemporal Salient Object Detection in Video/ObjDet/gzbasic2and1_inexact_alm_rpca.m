function [A_hat E_hat iter] = gzbasic2and1_inexact_alm_rpca(D, lambda, tol, maxIter)
...
while ~converged       
    ...
    for j = 1:n,
        frameTemp = temp_T(:,j);
        Blocknorm = norm(frameTemp,1);
        
        if Blocknorm < lambda/mu
               normshrink = 0;
               frameTemp=frameTemp*normshrink;
        else
               normshrink = (Blocknorm - lambda/mu);
               frameTemp=frameTemp*normshrink/Blocknorm;
        end
