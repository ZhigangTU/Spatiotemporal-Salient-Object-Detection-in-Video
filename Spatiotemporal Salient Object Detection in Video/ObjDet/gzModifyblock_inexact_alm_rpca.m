function [A_hat E_hat iter] = gzModifyblock_inexact_alm_rpca(D, lambda, tol, maxIter)
...
while ~converged       
    ...
    for j = 1:n,
        ...
        for jw = 1:nWblock,
            ...
            if Blocknorm < lambda/(2*mu) %%half of the original threhold value
               normshrink = 0;
               BlockTemp=BlockTemp*normshrink;
            else
               for jhh = 1:2,
               for jww = 1:2,
                   Submatrix  = BlockTemp(1+(jhh-1)*nSetBlock/2:jhh*nSetBlock/2, 1+(jww-1)*nSetBlock/2:jww*nSetBlock/2);
                   SubVect    = reshape(Submatrix,nSetBlock*nSetBlock/4,1);
                   Subnorm    = norm(SubVect,1)/(nSetBlock*nSetBlock/4);
                   
                   if Subnorm < lambda/mu %%half of the original threhold value
                      Submatrix=Submatrix*0;
                   else
                       normshrink = (Subnorm - lambda/mu);
                       Submatrix  = Submatrix*normshrink/Subnorm;
                   end
                   BlockTemp(1+(jhh-1)*nSetBlock/2:jhh*nSetBlock/2, 1+(jww-1)*nSetBlock/2:jww*nSetBlock/2)=Submatrix;
