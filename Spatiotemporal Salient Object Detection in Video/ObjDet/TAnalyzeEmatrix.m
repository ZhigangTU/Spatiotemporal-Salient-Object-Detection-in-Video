function [Mask_matrix, E_blockmask] = TAnalyzeEmatrix(NoiseMatrix, H, W)
...
nH = H;
nW = W;
nSetBlock = 8;   %(nSetBlock=4, 8), the block size will be nSetBlock*nSetBlock
...
for j = 1:n,
  ...
  for jw = 1:nWblock    
      ...
      nhEnd = min(jh*nSetBlock, nH);
      nwStart = (jw-1)*nSetBlock+1;
      nwEnd = min(jw*nSetBlock, nW);
      BlockTemp = frameTemp(nhStart:nhEnd, nwStart:nwEnd);
  ...
  for jw = 1:nWblock,
      if E_energy(jh,jw) == 1
          ...
          nwStart=(jw-1)*nSetBlock+1;
          nwEnd = min(jw*nSetBlock, nW);
          OriNoiseTemp = frameTemp(nhStart : nhEnd, nwStart : nwEnd);
          OriNoiseTemp = abs(OriNoiseTemp);
          E_sub8by8energy(nhStart : nhEnd, nwStart : nwEnd)=OriNoiseTemp;
      end
  end
  end
...
%   imwrite(Esubimage, outputMask);
  ...
  for jw = 1:nWblock,
      ...
      nwStart=(jw-1)*nSetBlock+1;
      nwEnd = min(jw*nSetBlock, nW);
      maskTemp(nhStart : nhEnd, nwStart : nwEnd)=E_energy(jh,jw)*ones(nhEnd-nhStart+1,nwEnd-nwStart+1);
  end
  end
