function  [A_hat, E_hat, iter] = TAdaptiveThreshblock2and1norm_inexact_alm_rpca(D, E_blockmask, H, W)
%function [A_hat E_hat iter] = gzblock2and1norm_inexact_alm_rpca(D, lambda, tol, maxIter)
...
while ~converged       
    ...
    for j = 1:n,
        ...
        Emasktemp = reshape(E_blockmask(:,j),nHblock,nWblock);
        for jh = 1:nHblock,
        for jw = 1:nWblock,
            ...
            if Emasktemp(jh,jw)==0
                if Blocknorm < lambda/mu
                   normshrink = 0;
                   BlockTemp=BlockTemp*normshrink;
                else 
                  normshrink = (Blocknorm - lambda/mu);
                  BlockTemp=BlockTemp*normshrink/Blocknorm;
                end  
                frameTemp(nhStart : min(nhStart + nSetBlock-1, nH), nwStart : min(nwStart + nSetBlock-1, nW))=BlockTemp;
            end
            
            if Emasktemp(jh,jw)==1
                if Blocknorm < 0.1*lambda/mu
                   normshrink = 0;
                   BlockTemp=BlockTemp*normshrink;
                else 
                  normshrink = (Blocknorm - 0.1*lambda/mu);
                  BlockTemp=BlockTemp*normshrink/Blocknorm;
                end  
                frameTemp(nhStart : min(nhStart + nSetBlock-1, nH), nwStart : min(nwStart + nSetBlock-1, nW))=BlockTemp;
            end
