function [A_hat E_hat iter] = gzKnownblockparts2and1norm_inexact_alm_rpca(D, lambda, tol, maxIter)
% Oct 2009
...
while ~converged       
    ...
    for j = 1:n,
        ...
        frameCopy = zeros( nH, nW);
        
        nhStart=220-(j-10);
        nwStart=290;
        
        BlockTemp = frameTemp(nhStart : nhStart + nhSetBlock, nwStart : nwStart + nwSetBlock );
        ...
        frameCopy(nhStart : nhStart + nhSetBlock, nwStart : nwStart + nwSetBlock)=BlockTemp;
        frameTemp = reshape(frameCopy,nW*nH,1);
        E_hat(:,j) = frameTemp;
    end
