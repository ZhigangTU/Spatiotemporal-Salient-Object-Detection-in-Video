function [frameIOMap, frameObjectMap, frameObjectBoxComb, frameEnergy] = SaliencyObject(flowdata,magflow,framedata,height,width,nframe,outPath)
...
videofolder = '012';
options.infolder = fullfile('E:\Action Recognition\Pose-CNN\Data\UCF-Sports\images', 'SkateBoarding-Front', videofolder);   %JHMDB\images\brush_hair; data\input\

% The folder where all the outputs will be stored.
options.outfolder = fullfile(currentPath, 'data', 'output\RPCA\SkateBoarding-Front', videofolder);
...
%     frameName = data.names{index};    
    ...
    %% Method 1 -- Appearnce Boundary --> Probability of the magnitude of the input image(Not the boundary logical labels). Static appearance boundary --> edge_canny 
    G = 0.1*edge_detect(framex); 

    % Motion Boundary
%     Flowgradient = getFlowGradient(flow);
%     magnitude = getMagnitude(Flowgradient);
...
%     %%%-> Method 1 -- magnitude of the gradient of the optical flow field (gradBoundary) <-%%%
%         gradBoundary = 1 - exp(-options.gradLambda * magnitude);                
%         if (max(magnitude(:)) < 10)
%             gradBoundary = gradBoundary + 0.01;
%         end
%         G = G.*(gradBoundary);     % MoBoundary = gradBoundary*rotBoundary (original-->G.*(gradBoundary))

    % Method 2 -- gradBoundary + rotBoundary; + G --> (Whole object) 
    mode = 3;
    if (max(magnitude(:)) >= 1)
