function [A_hat E_hat iter] = gzfixagainblock2and1norm_inexact_alm_rpca(D, lambda, tol, maxIter)
% Oct 2009
...
nW= 4*sqrt(m/12);
nH= 3*sqrt(m/12);
nwSetBlock=160; %the block size will be nSetBlock*nSetBlock
nhSetBlock=120;
nWblock=nW/nwSetBlock;
nHblock=nH/nhSetBlock;
...
while ~converged       
    ...
    for j = 1:n,
        ...
        for jh = 1:nHblock,
        for jw = 1:nWblock,
            nhStart=(jh-1)*nhSetBlock+1;
            nwStart=(jw-1)*nwSetBlock+1;
            BlockTemp = frameTemp(nhStart : nhStart + nhSetBlock -1, nwStart : nwStart + nwSetBlock -1);
            %Blocknorm = norm(BlockTemp,'fro')/sqrt(nwSetBlock*nhSetBlock);
            %Blocknorm = norm( BlockTemp(:), inf);
            %Blocknorm = norm( BlockTemp(:), 1)/(nwSetBlock*nhSetBlock);
            ...
            if jh==2 & jw==1 
               ...
               %end
               BlockTemp1 = max(BlockTemp - setratio*lambda/mu, 0);
               BlockTemp = BlockTemp1+min(BlockTemp + setratio*lambda/mu, 0);
            else 
               %normshrink = 0;
               %BlockTemp=BlockTemp*normshrink;
               setratio=1;
            ...
            frameTemp(nhStart : nhStart + nhSetBlock-1, nwStart : nwStart + nwSetBlock-1)=BlockTemp;
        end
        end
